...
\section{Equations de Maxwell à l'extérieur avec condition limite de Calderòn}
  ...
  \subsection{Expression de l'opérateur de Calderòn pour un plan infini en tant que multiplicateur de Fourier}
    ...
    On remarque que l'opérateur de Calderòn n'est pas un opérateur aux dérivées partielles puisque son multiplicateur de Fourier fait intervenir la racine carré \(\sqrt{k^2-k_x^2 - k_y^2}\), qui n'est pas un polynôme en $(k_x,k_Y)$ et donc, par transformée inverse de Fourier ne peut s'exprimer avec les opérateurs \(\partial/\partial x, \partial / \partial y\).
    ...
    Par exemple si on approche ce multiplicateur de Fourier par le quotient \(\frac{Q(k_x,k_y)}{R(k_x,k_y)}\), écrire \({\hat v}(k_x,k_y)=\frac{Q(k_x,k_y)}{R(k_x,k_y)}{\hat u}(k_x,k_y)\) se réécrit (même si il faut des résultats supplémentaires pour que la formulation qui suit lui soit équivalente) \(R(k_x,k_y){\hat v}(k_x,k_y)=Q(k_x,k_y){\hat u}(k_x,k_y)\), qui se transforme en opérateurs différentiels.
  ...
  \subsection{Expression de l'opérateur de Calderòn pour un cylindre infini}
    ...
    %     \begin{lemma} The Calderòn operator for the cylindrical annulus is given by
    %     $$
    %         \begin{pmatrix}
    %             -H_{\theta}
    %             \cr
    ...
    %     \begin{lemma}
    %     \label{Z-lemme}
    %     For the cylindrical annulus, the Calderòn operator is characterized, under the conditions $d_n(R)(dd_n)'(R)\not=0$ for all $n$, and $k_3\not=0$, by
    %     $$
    %     \begin{pmatrix}
    %         H_{\theta}
    %         \\
    ...
    \subsubsection{Expression de l'opérateur de Calderòn pour \(k_z\) quelconque}
      ...
      % La condition de conducteur parfait s'exprime sur \(\vE\), donc l'opérateur \(\hat\vJ = (\hat\mZ_n(k_z))^{-1}\hat\vE_t\) est plus simple à exprimer.
      ...
      % \begin{align*}
      %   (\hat\mZ_n(k_z))^{-1}_{11} ={}& i\frac{k_3}{k\eta}\Sn,
      %   \\
      %   (\hat\mZ_n(k_z))^{-1}_{12} ={}& i\frac{nk_z}{kk_3r_1\eta}\Sn,
      %   \\
      %   (\hat\mZ_n(k_z))^{-1}_{21} ={}& i\frac{nk_z}{kk_3r_1\eta}\Sn,
      %   \\
      %   \begin{split}
      %   (\hat\mZ_n(k_z))^{-1}_{22} ={}& - i\frac{k}{k_3\eta}\Tn
      %   \\
      %   {}& + i\frac{n^2k_z^2}{kk_3r_1^2\eta}\Sn.
      %   \end{split}
      % \end{align*}
...
\section[Problème extérieur et condition limite de Calderòn]{Equations de Maxwell à l'extérieur de l'objet avec une approximation de l'opérateur de Calderòn sur son bord}
  ...
  \subsection{Taylor à l'ordre 1: la CI01}
    ...
    On approche l'opérateur de Calderòn par
    \[
...
\section{Etude de l'unicité des solutions du problème extérieur avec CIOE sur le bord}
  ...
  Il est bien entendue que pour chaque \gls{acr-cioe}, le problème avec une onde incidente, conduit \emph{s'il y a unicité}, à une unique solution diffractée dont on espère qu'elle est une approximation de la solution exacte du problème diffracté avec opérateur de Calderòn.

  {~}\\{~}
  Tout le problème est de choisir les paramètres complexes des CIOE, obtenues grâce à des approximations issues de critères physiques (matériaux, courbure et fréquence) mais non issus de la solution du problème diffracté avec opérateur de Calderòn. 

  Il est inutile de considerer des paramètres qui ne permettraient pas d'obtenir une unique solution.
  ...
  S'il n'y a pas unicité, il est aussi clair que pour un problème avec source, on peut ne pas avoir existence.
  ...
  {~}\\{~}
  Nous allons donc assurer de l'unicité de la solution du problème sans source pour déterminer des \glspl{acr-csu} qui piloterons le choix des paramètres, mais il est extrèmement important de comprendre que grâce à cela, nous pourrons \emph{garantir} l'unicité de la solution du problème avec source.
  ...
  Afin de garantir la propositon \ref{prop:cgu}, les conditions suivantes permettent de choisir les paramètres des CIOE correspondantes.
