...
\section{Equations de Maxwell à l'extérieur avec condition limite de Calderon}
    ...
    \subsection{Expression de l'opérateur de Calderon pour un cylindre infini}
        ...
        \end{REM}
            ...
            Pour un cylindre de rayon \(r_0 [m]\) recouvert d'une couche d'épaisseur \(d [m]\) (on note \(r_1=r_0 + d\)) de matériau diélectrique de constante relative \(\eps,\mu\) et donc de nombre d'onde complexe \(k = k_0\sqrt{\eps\mu}\) et d'impédance relative \(\eta=\sqrt{{\mu}/{\eps}}\), l'opérateur de Calderon s'exprime tant que coefficient de série de Bessel et multiplicateur de Fourier (en \(k_z\)) matriciel. {\it en tant que multiplicateur de Fourier en  \(k_z\), n}: 
            \begin{REM}
            en effet $n$ est aussi un indice de Fourier car la transformée de Fourier discrète est aussi une vraie transformé de Fourier. Mets le
\end{REM}
            \subsubsection{Expression de l'opérateur de Calderon quand \(k_z=0\)}
              Le cas de l'incidence perpendiculaire à l'axe du cylindre simplifie grandement les calculs.
