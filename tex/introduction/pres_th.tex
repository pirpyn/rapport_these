...
% Nous traitons le problème des équations de Maxwell harmoniques, en convention \(e^{i\w t}\) pour la dépendance temporelle.

% Soit un objet \(\OO\) de type conducteur parfait, recouvert d'une couche matériau diélectrique d'épaisseur \(d\), caractérisé par sa permittivité diélectrique \(\gls{phy-eps}\) et sa perméabilité magnétique \(\gls{phy-mu}\) qui peuvent dépendre du point courant et être complexe, l'ensemble se situant dans le vide tel que la célérité d'une onde électromagnétique soit \gls{phy-c}, et les constantes de permittivité et de perméabilité dans ce domaine soient \(\gls{phy-eps}_0\), \(\gls{phy-mu}_0\) avec \(\eps_0\mu_0 = c^{2}\).
...
% \end{equation}
% A l'extérieur de \(\OO\), les paramètres sont \(\eps_0,\mu_0\) et à l'intérieur \(\eps,\mu\).

% Nous complétons ces équations par des conditions aux limites à la surface \(\Gamma\) de \(\OO\), supposée régulière et dont on note la normale sortante \(\vn\):
% \begin{itemize}
