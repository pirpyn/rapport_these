\section{Cas d'un objet plan infini}
        ...
        \begin{figure}[!hbt]
            ...
            \begin{tikzpicture}[scale=1]
                \begin{axis}[
                    ...
                    \addplot [black] table [col sep=comma, x={s1}, y={Im(z_ex.tm)}] {tikz/csv/impedance/HOPPE_33/HOPPE_33.z_ex.P.csv};
                    \addlegendentry{TM};

                    \addplot [black,dashed] table [col sep=comma, x={s1}, y={Im(z_ex.te)}] {tikz/csv/impedance/HOPPE_33/HOPPE_33.z_ex.P.csv};
                    \addlegendentry{TE};
                \end{axis}
            \end{tikzpicture}
        ...
        \begin{figure}[!hbt]
            ...
            \begin{tikzpicture}[scale=1]
                \begin{axis}[
                    ...
                    \addplot [black] table [x={s1}, y={Im(z_ex.tm)},col sep=comma] {tikz/csv/impedance/ICEAA_11/ICEAA_11.z_ex.P.csv};
                    \addplot [black,dashed] table [x={s1}, y={Im(z_ex.te)},col sep=comma] {tikz/csv/impedance/ICEAA_11/ICEAA_11.z_ex.P.csv};
                \end{axis}
            \end{tikzpicture}
            \begin{tikzpicture}[scale=1]
                \begin{axis}[
    ...
    \subsection{Opérateur d'impédance pour plusieurs couches}
        ...
        On rappelle que \(\det{\mC_m} = \frac{k_{3m}^2}{\eta_m^2}\) et \(\mC_m^{-1}= \frac{k_m\eta_m}{k_{3m}^2}\left(k_m^2 \mI + \hat{\mLD}\right)\).
  ...
  \subsubsection{Coefficients de réflexions}
    ...
    \begin{prop}
      On définit la matrice \(\mR\) telle que pour tout point de la surface \(z=0\)
      \begin{equation}
      ...
      \begin{equation}
          \mR = 
          \left(k_0^2\mI + \hat{\mLD} + k_0k_{3,0}\hat{\mZ}_m\right)^{-1}\left(k_0^\mI + \hat{\mLD} - k_0k_{3,0}\hat{\mZ}_m\right)
      \end{equation}
    \end{prop}
    ...
    \begin{proof}
        ...
        \begin{align}
            ...
            \\
            \vect{e_z}\pvect \hat{\vH}(k_x,k_y,0_+) &= i\mC_{m+1}\left(\vect{C_1}+\vect{C_2}\right)
        \end{align}
        et que 
        \begin{align}
            ...
            \\
            \intertext{donc}
            ik_{3,0}\left(\vect{C_1}-\vect{C_2}\right) &= i\hat{\mZ}_m\mC_m\left(\vect{C_1}+\vect{C_2}\right)
            \\
            \intertext{soit en factorisant par chaque vecteur}
            \left(k_{3,0}\mI - \hat{\mZ}_m\mC_m\right)\vect{C_1} & = \left(k_{3,0}\mI+\hat{\mZ}_m\mC_m\right)\vect{C_2}
        \end{align}
        et on remplace \(\mC_{m+1}^{-1}\) par son expression avec \(\hat{\mLD}\) à l'extérieur
        \begin{equation}
    ...
    \begin{figure}[!hbt]
        ...
        \begin{tikzpicture}[scale=1]
            \begin{axis}[
                ...
                \addplot [black] table [x={s1}, y={Abs(r_ex.tm)},col sep=comma] {tikz/csv/impedance/ICEAA_11/ICEAA_11.r_ex.P.csv};
                \addlegendentry{TM};
                \addplot [black,dashed] table [x={s1}, y={Abs(r_ex.te)},col sep=comma] {tikz/csv/impedance/ICEAA_11/ICEAA_11.r_ex.P.csv};
                \addlegendentry{TE};
            \end{axis}
        \end{tikzpicture}
        \caption[Reproduction résultat P. Soudais]{\(\eps = 4, \mu = 1, d=0.035\text{m}, f=12\text{GHz}\)}
