\section[Opérateur de Calderón pour un cylindre infini]{Expressions exactes des matrices d'impédance et des matrices de réflexion pour un cylindre infini}
  ...
  \subsection{Expressions des champs tangentiels dans chaque couche}
    ...
    \begin{prop}
      Dans chaque couche \(r_{m-1}\le r < r_m\), il existe \((c_i(n,k_z))_{1\le i\le 4}\) telles que les composantes en \(\vect{e_\theta},\vect{e_z}\) des champs (\(\vE,\vJ=\vect{e_r}\pvect\vH\)), dites composantes tangentielles par abus de langage, sont
      \begin{subequations}
        \begin{align}
