...
\section{Calcul des coefficients des CIOEs}\label{sec_coeffs_cioe}
%{
...
\TODO{Faire le ménage dans cette étude, il y a des choses non finis et/ou inutile}
\subsection{Etude de la forme quadratique}\label{sec_etud_Q}
...
% \begin{align}
%   \det(\tilde{Q_j} -\lambda) = \det(A-\lambda) \det(A-\lambda - B(A-\lambda)^{-1}B^t)\label{eq_detQj}
% \end{align}
...
\begin{align}
\label{eq_contr_1}|b' - b| - \epsilon \geq 0 \\
\label{eq_contr_2}|b'' - b| - \epsilon \geq 0
\end{align}
Pour la seconde, c'est évident. Rappel: $c_3 = a_1''-b'a_0''+b''a_0'$
\begin{align}
\label{eq_contr_3}|c_3| - \epsilon \geq 0
\end{align}
Et enfin les deux dernières sont déjà sous forme d'inégalités positives
\begin{align}
\label{eq_contr_4}c_3(a_0'a_1'' - a_0''a_1') \geq 0 \\
\label{eq_contr_5}c_3b'' \geq 0
\end{align}
...
\grad \eqref{eq_contr_1} &= (0,0,0,0,-1,0)^t + \grad b\\
\grad \eqref{eq_contr_2} &= (0,0,0,0,0,-1)^t + \grad b\\
\end{align}
\begin{align}
\notag \grad c_3 &= (b'',-b',0,1,-a_0'',a_0')^t\\
\grad \eqref{eq_contr_3} &= \grad c_3 \\
\grad \eqref{eq_contr_4} &= (a_0'a_1'' - a_0''a_1')\grad (c_3) + c_3(a_1'',-a_1',-a_0'',a_0',0,0)^t \\
\grad \eqref{eq_contr_5} &= b''\grad (c_3) + c_3(0,0,0,0,0,1)^t
\end{align}
\fi
