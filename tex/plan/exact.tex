\section[Opérateur de Calderón pour un plan]{Expressions exactes des matrices d'impédance et des matrices de réflexion pour un plan infini}
    ...
    \begin{prop}
        ...
        \begin{REM}
  manque il
\end{REM}existe \((c_i(k_x,k_y))_{1\le i \le4}, \in (\CC(\RR^2))^4\) tels que les champs solutions de \ref{eq:imp_fourier:intro:maxwell_harmonique:form_cea} sont de la forme
        \begin{align*}
        ...
        \end{align*}
        Les constantes \(c_i\) ne peuvent en réalité pas être choisies arbitrairement, car les exponentielles complexes divergent quand \(k_x^2 + k_y^2 \rightarrow \infty\) et ces solutions n'ont pas de transformées de Fourier. Nous supposons que les constantes \(c_i\) satisfassent 
        \begin{REM}
  vérifient ? satisfassent c'est beurk
\end{REM}
        la propriété \eqref{prop:unicite:interieur:postulat:multi-couche}, où il n'y a pas de résonances.
        % \begin{REM}
...
\end{REM}
        ...
        Voir par exemple la proposition \ref{prop:unicite:interieur:postulat:multi-couche} pour un exemple 
\begin{REM}
  donc, ce qui est vrai, on identifie l'existence et unicité d'un champ vérifiant les équations de maxwell harmoniques avec condition de dirichlet sur le bord de l'obstacle à l'existence de l’opérateur de Calderón.
\end{REM}
        de condition nécessaire d’existence pour 2 couches.
      \end{remark}
      ...
      \begin{prop}
          Il existe \((c_i(k_x,k_y,z))_{1\le i \le4}\) constants par morceaux en \(z\) tels que les composantes en \(\vect{e_x},\vect{e_y}\) des champs, dites composantes tangentielles par abus de langage,
\begin{REM}
  ???
\end{REM}
           sont
      ...
      \begin{proof}
        ...
        On reprend les équations de Maxwell appliquées sur les transformées de Fourier.
\begin{REM}
  Ok, c'est quoi ça ???
\end{REM}

        À partir des composantes \(\vect{e_x},\vect{e_y}\) des équations de Maxwell, on  détermine un système d'\glspl{acr-edo}
\begin{REM}
  pluriel
\end{REM}

         sur \(\hat{E_x},\hat{E_z},\hat{\mathcal{H}_x},\hat{\mathcal{H}_z}\) à partir de \(\hat{E_z}, \hat{\mathcal{H}_z}\).
        % \begin{REM}
...
\end{REM}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        \begin{lemme}[Continuité des impédances]
          ...
          On suppose qu'il existe des matrices
\begin{REM}
  mal dit ? c'est quoi ? la régularité de Et ?
\end{REM}
          de part et d'autre de l'interface telles que
          \begin{align*}
...
\begin{REM}
 surligné: on utilise la continuité des champs
\end{REM}
\begin{REM}
  ce qui s'identifie à Z..; : je ne comprend pas la preuve, ça tourne en rond ?
\end{REM}
          \begin{align*}                
...
\begin{REM}
de ( mieux que le chauffard!!! condition au ... )  
\end{REM}
            \begin{align*}
            ...
            \end{align*}
\begin{REM}
  c'est ça le résulat non? pourquoi il n'est pas énoncé ? il faut le mettre dans la proposition!!!  
\end{REM}
        \end{proof}
        ...
        Dans l'article de \cite{marceaux_high-order_2000}, la matrice d'impédance est telle que, soit \(\vK = \vn \pvect \vE\)
        \begin{align*}
...
\begin{REM}
  aux
\end{REM}
       sur le bord supérieur d'une couche détermine la condition limite
\begin{REM}
  aux
\end{REM}
...
\subsection{Applications numériques}
  ...
  Dans cet ouvrage, l'opérateur de Calderón considéré n'est pas le même
\begin{REM}
  ca veut dire quoi ça ? je ne comprends rien !!!
\end{REM}
   ( voir annexe \ref{sec:annexe:hoppe} ) et on pose \(\hat{\mathfrak{Z}} = \hat{\mZ}\begin{bmatrix}0&1\\-1&0\end{bmatrix}\) la matrice d'impédance utilisée par Hoppe.
...
\begin{REM}
  pas clair, c'est quoi les résultats de soudais ?
\end{REM}
  \begin{figure}[!hbt]
      \centering
