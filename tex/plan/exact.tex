\section[Opérateur de Calderón pour un plan]{Expressions exactes des matrices d'impédance et des matrices de réflexion pour un plan infini}
    ...
    \subsection{Expressions des champs tangentiels dans chaque couche}
      ...
      \begin{remark}
        Nous supposerons dans ce qui suit certaines matrices inversibles ( en autres, \(\mB_E(k_x,k_y,-d^+)\), voir plus loin pour la définition de cette dernière ).
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        \begin{lemme}[Continuité des impédances]~{}
          ...
          Soient \(({\vE_t},{\vH}_t)\) des champs solutions du problème de Maxwell dans les deux couches, alors si l'on peut définir des matrices qui lient les traces tangentielles des champs à l'interface de ces couches, alors ces matrices sont identiques sur l'interface.
        ...
        \begin{prop}
          ...
          \end{align*}
          où \(\hat \mZ(k_x,k_y)\) est donné dans la définition précédente.
        \end{prop}
        ...
        Dans l'article de \cite{marceaux_high-order_2000}, la matrice d'impédance est telle que, soit \(\vK = \vn \pvect \vE\) alors on a
        \begin{align*}
  ...
  \subsection{Expression de la matrice d'impédance pour plusieurs couches}
    ...
    On suppose qu'il n'y pas de résonances donc que la condition nécessaire à l'existence de l'opérateur de Calderón est vérifié.
...
% \end{REP}
    ...
    Nous allons donc pouvoir exprimer l'approche itérative : déterminer la relation liant les champs électromagnétiques d'une couche à l'autre.
...
\subsection{Applications numériques}
  ...
  On remarque que le symbole de Hoppe est dimensionné ( en Ohm ) tandis que le nôtre non. De plus les ondes évanescentes, c'est-à-dire telles que \(k_x > k_0\) sont prisent en compte sur ce graphe. En reproduisant ces résultats, nous nous assurons que notre approche itérative est correctement codée.

  On applique les hypothèses précédentes d'invariance par rotation, donc \(k_y=0\), sachant que pour \(k_x\slash k_0=2\), \(k_3(k_x,0)\) est nul et l'impédance n'est pas définie en ce point. 
  ...
  \begin{figure}[!hbt]
    ...
    \caption[Partie imaginaire de l'opérateur de Calderón pour une couche plane de matériau issue de Hoppe \& Rahmat-Samii p.~33, avec ondes évanescentes]{Partie imaginaire des coefficients diagonaux de \(\hat{\mathfrak Z}\) pour \(\eps = 4, \mu = 1, d=0.015\text{m}, f=1\text{GHz}\)}
  ...
  \begin{figure}[!hbt]
    ...
    \caption[Partie imaginaire de l'opérateur de Calderón pour une couche plane de matériau de P.~Soudais p.~11, avec asymptote]{Partie imaginaire des coefficients diagonaux de \(\hat\mZ\) pour \(\eps = 4, \mu = 1, d=0.0035\text{m}, f=12\text{GHz}\)}
