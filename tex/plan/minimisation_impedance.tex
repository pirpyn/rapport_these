...
\end{REM}
    ...
    \begin{prop}
      Soit \(X = (a_0,a_1,a_2,b_1,b_2)\), \((k_x,k_y)\) fixé et \(\hat\mZ_{ex}\) l'opérateur
\begin{REM}
on dirait que j'ai la réponse à ma question
\end{REM}
       d'impédance exact du plan.
...
le
\end{REM}
       vecteur colonne complexe tel que
      \begin{equation*}
        g = 
        \begin{bmatrix}
...
\end{REM}
      ...
      Supposons \(t\) et 1 colinéaires, c'est
\begin{REM}
ce 
\end{REM} qui revient à ne prendre qu'un seul couple \(k_x,k_y\).
      ...
      Nous l'avons vérifié numériquement et pour nous
\begin{REM}
supprimer
\end{REM}
       nos empilements, nous n'avons jamais constaté que ce déterminant s'annulait.
...
\end{REM}
      ...
      % \begin{REP}
        ...
        Tout comme pour la CI1, nous avons vérifié numériquement que sur un certain nombre d'empilements et d’incidences qu'il ne s'annule pas.
\begin{REM}
  que qu'il
\end{REM}
...
\end{REM}
    ...
    \subsubsection{Minimisation sans contraintes}
      ...
      \begin{defn}[Minimisation sans contraintes pour la CI3]
...
\end{REM}
      ...
      \begin{proof}
        ...
        \end{align*}
        Si la matrice \(\conj{\mA_{CI}^t}\mA_{CI}\) est inversible, comme elle est par définition hermitienne et positive, alors la fonction
\begin{REM}
  laquelle ? J
\end{REM}

         est quadratique. Il existe donc un unique minimum où le gradient de cette fonction s'annule. Or
        \begin{align*}
...
\end{REM} 
      ...
      Si la fonction \(J_Z\) est quadratique et continue, alors il y a existence.
      \begin{REM}
  de quoi
\end{REM} 
...
\end{REM} 
      ...
      L'unicité dépend en plus de la convexité de l'espace des contraintes, ce qui est vérifié par les CIOE (CI0, CI01, CI4).
  ...
  \subsection{Résultats numériques sans contraintes}
      ...
      La figure \ref{fig:imp_fourier:plan:soudais:hoibc} permet de vérifier les résultats de \cite[p.~11]{soudais_3d_2017} pour une couche de matériau
\begin{REM}
  matériaux
\end{REM} 
       sans perte où l'on remarque une asymptote, que nous déterminerons plus tard.
