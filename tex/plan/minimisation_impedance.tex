\section{Calcul des coefficients de la CI3 par moindres carrés sur l'impédance}
  ...
  \subsection{Calcul des coefficients pour une incidence}
    ...
    Les fonctions associées aux CIOE CI0, CI01, CI4 se déduisent aisément.


  \subsection{Balayage en incidences et moindres carrés}

    Nous allons donc réaliser une balayage sur les variables de Fourier et exhiber un calcul permettant d'obtenir ces coefficients.
    Par abus de langage, nous ne parlerons de moindres carrés que lorsqu'il y a plusieurs couples \((k_x,k_y)\), alors que par définition, le problème pour une incidence est aussi un problème des moindres carrés.
  ...
  \subsection[Inversibilité des matrices Mci]{Inversibilité des matrices \(\mM_{CI}\)}
    ...
    \subsubsection{Pour la CI4}
      ...
      Soit \(s\) le vecteur de \(\CC^{N_{k}}\) tel que \(s_i = \ps{\mI}{\hat{\mZ}(k_{x},k_{y})_i}_F\) et \(z\) le vecteur de \(\RR^{N_{k}}\) tel que \(z_i = \norm{\hat{\mZ}(k_{x},k_{y})_i}_F\). On a l'expression suivante de \({\mM_{CI1}}\)
      ...
      \end{REM}
      \begin{REP}
        EN COURS; BASE DAPRES LA CI3; IDEE METTRE Ti dans Si et Zi.

        Soit \(s\) le vecteur de \(\CC^{N_{k}}\) tel que \(s_i = \ps{\mI}{t_i\hat{\mZ}(k_{x},k_{y})_i}_F\) et \(z\) le vecteur de \(\RR^{N_{k}}\) tel que \(z_i = \norm{t_i\hat{\mZ}(k_{x},k_{y})_i}_F\).
        ...
        \begin{equation*}
          ...
          \\
          \ps{1}{\conj{s}}  & -\ps{t}{\conj{s}} & \ps{z}{z}
          \end{bmatrix}.
        \end{equation*}
    ...
    \subsubsection{Pour la CI3}
      ...
      \begin{REP}
        ...
        Le nombre de couches importe peu : cela n'influe que sur l'expression de \(Z_{ex}\), qui est déjà assez complexe avec une.
      ...
      \begin{REP}
        ...
        Nous renvoyons aux définitions des vecteurs de \(\RR_+^{N_{k}}\) \(t\) et \(1\) introduits ci-avant.
        Soit \(s_D,s_R\) les vecteurs de \(\CC^{N_{k}}\) et \(z_D,z_R\) les vecteurs de \(\RR_+^{N_{k}}\) tels que
        \begin{align*}
          s_{Di} &= \ps{\mLD}{\hat{\mZ}(k_{x},k_{y})_i}_F
          \\
          s_{Ri} &= \ps{\mLR}{\hat{\mZ}(k_{x},k_{y})_i}_F
          \\
          z_{Di} &= \norm{\mLD\hat{\mZ}(k_{x},k_{y})_i}_F
          \\
          z_{Ri} &= \norm{\mLR\hat{\mZ}(k_{x},k_{y})_i}_F
        \end{align*}
        ...
        \begin{equation*}
          ...
          \\
          -\ps{1}{\conj{s_D}} & \ps{t}{\conj{s_D}}  & 0                    & \ps{z_D}{z_D} & 
          \\
          \ps{1}{\conj{s_R}}  & 0                   & -\ps{t}{\conj{s_R}}  &             & \ps{z_R}{z_R}
          \end{bmatrix}.
        \end{equation*}
