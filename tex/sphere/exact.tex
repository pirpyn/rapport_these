...
\section{Expressions exactes des matrices d'impédance et des matrices de réflexions pour une sphère}
  ...
  \subsection{Expression de la matrice d'impédance pour une couche}
    ...
    \begin{proof}
      On se place dans le matériau: \(r_0 \le r \le r_ 1 \).

      On applique la condition limite du conducteur parfait \(\hat \vE(r_0,n) = 0\) dans \eqref{prop:imp_fourier:sphere:Et}. Comme les harmoniques sphériques forment une base de \(L^2(S(r_0))\), chaque terme doit être nul.
      \begin{equation*}
        \mj_{E}(r_0,n)
        \begin{bmatrix}
          a_{nm} \\
