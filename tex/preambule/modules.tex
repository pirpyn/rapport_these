...
\usepackage[Lenny]{fncychap}
    \makeatletter
        \ChNameVar{\huge} % On modifie la taille du mot Chapitre
    \makeatother
...
\usepackage{pgfplots}
    ...
    \tikzexternalize{thesis_payen} % Cette commande doit être placé dans le main.tex pour qu'Overleaf puisse fonctionner 
    ...
    %     thick, solid,          every mark/.append style={solid}\\% style par default noir et blanck qui boucle
    % }
    % Parametres généraux sur les figures
    \pgfplotsset{%
        compat=newest,
        height=6.5cm, % hauteur fixe
        % cycle list name=my cycle, % Pour définir le style par défault
        every axis legend/.style={
            cells={anchor=west}, % texte aligné à hauche
            draw=black, % de couleur noire
        },
        % Style des graphes
        every axis plot/.append style={
            mark options=solid, % symbole plein
            line width=1pt % épaisseur du trait
        },
    }

    \newcommand{\ccio}{blue}
    \newcommand{\cciou}{green}
    \newcommand{\cciu}{orange}
    \newcommand{\ccit}{red}
    \newcommand{\ccitc}{gray}
    \newcommand{\ccis}{purple}
