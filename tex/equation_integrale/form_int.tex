...
\section{Discrétisation de la surface de l'objet}
  ...
  Soit \(N=\frac{3}{2}N_e\) le nombre d'arête
\begin{REM}
  +s
\end{REM} 
   du maillage.
  On désigne par \(\vn\) la normale unitaire sortante du maillage, non défini
\begin{REM}
  +e
\end{REM} 
   aux nœuds et arêtes du maillage.
  ...
  \subsection[Fonctions de Raviart-Thomas phi Hdiv-conforme]{Fonctions de Raviart-Thomas \(\vect{\phi}\) \(\Hdiv\)-conforme}
    ...
    Nous présentons ici les fonctions Raviart-Thomas de degré 0, c'est à dire
\begin{REM}
  tiret
\end{REM} 
     telles que les degrés de liberté soient au milieu de chaque arête (voir \cite[eq.~(3.10)]{raviart_mixed_1977}).
...
\section{Fonctions de projections}
  ...
  \subsection[Fonctions de Bendali q]{Fonctions de Bendali \(\vect{q}\)}
    ...
    Nous modifions ces fonctions en les divisant par 2 fois l'aire du triangle.
\begin{REM}
  mal dit
\end{REM} 
...
\section{Matrices de projections}
  ...
  \end{prop}
  \begin{proof}
    C'est une conséquence immédiate du produit triangulaire
\begin{REM}
  c'est vraiment son nom ? mixte ?
\end{REM} 
     \(a\cdot (b \pvect c) = c\cdot (a \pvect b)\) et que les fonctions de bases soient tangentes à la surface.
    \[
      \begin{aligned}
...
\begin{REM}
  par
\end{REM} 
   abus de notation, nous omettrons le \(-1/2\) dans l'espace \(\Hdiv^{-1/2}(\Gamma)\equiv\Hdiv(\Gamma)\).
...
\section{Contribution de la CIOE dans la discrétisation de la forme variationnelle}
  ...
  Il reste alors à trouver une bonne approximation de \(\vE_t\) et \(\vH_t\) pour exprimer \( \frac{1}{2} \int_{\Gamma_h} \vE_t(\vx) \cdot \phii(\vx) \dd{\Gamma}(\vx)\) et \(\frac{1}{2} \int_{\Gamma_h} \vH_t(\vx) \cdot \phii(\vx) \dd{\Gamma}(\vx)\)
\begin{REM}
  s
\end{REM} en fonctions
   des inconnues \(\vJ, \vK\).
  Selon la CIOE employée, cette décomposition ne sera pas la même.
  Notre but est alors de trouver une formulation qui se comporte bien quand on déduit d'une CIOE un
\begin{REM}
  une
\end{REM} 
   autre plus simple.
...
\end{REM} 
  ...
  Les résultats numériques qui suivent sont issus d'un code maquette où le système linéaire est codé tel quel, ce qui impose des limites évidentes en coût mémoire.
\begin{REM}
  ???
\end{REM} 
  L'introduction de cette modification dans un code industriel "haute performance" est une tâche complexe qui nécessite un investissement conséquent des équipes en charge de ce solveur, et n'a pas été réalisé dans le cadre de cette thèse.