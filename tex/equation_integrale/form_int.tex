\section{Équations intégrales}
    ...
    \subsection{EFIE}
      ...
      \begin{defn}
        ...
        \(\vJ,\vK\) sont solutions des équations intégrales en champ électrique (\gls{acr-efie}) s'il existe \((\vJ,\vK) \in \Hdiv(\Gamma_h)^2\) tels que
        \begin{multline*}
          \vE^{inc}_t(\vx) =
            \frac{\vE_t(\vx)}{2}
    ...
    \subsection{MFIE}
      ...
      \begin{defn}
        ...
        \(\vJ,\vK\) sont solutions des équations intégrales en champ magnétique (\gls{acr-mfie}) s'il existe \((\vJ,\vK) \in \Hdiv(\Gamma_h)^2\) tels que
        \begin{multline*}
          \vH^{inc}(\vx) =
          \frac{\vH_t(\vx)}{2}
...
\section{Matrice de changement de base}
  ...
  \begin{proof}
  ~
  %\subsection[Méthode de Stupfel 2015]{Méthode de \cite{stupfel_implementation_2015}}
  ...
  \begin{enumerate}
    ...
    %\subsection{Deuxième méthode, fausse}
