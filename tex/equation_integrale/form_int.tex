    ...
    \subsection{EFIE}
      ...
      \begin{prop}
        ...
        \begin{multline*}
          \vE^{inc}_t(\vx) =
            \frac{\vE_t(\vx)}{2}
              - \int_{\Gamma} \vgrad g(\vx - \vy) \pvect \vK(\vy) \dd{\Gamma}(\vy) \cdots \\
    ...
    \subsection{MFIE}
      \begin{prop}
        ...
        \begin{multline*}
          \vH^{inc}(\vx) =
          \frac{\vH_t(\vx)}{2}
            - \int_{\Gamma} \vgrad g(\vx - \vy) \pvect \vJ(\vy) \dd{\Gamma}(\vy) \cdots \\
...
\section{Contribution de la CIOE dans la discrétisation de la forme variationnelle}
  ...
  \subsection[Discrétisation des opérateurs LD, LR]{Discrétisation des opérateurs \(\LD,\LR\)}
    ...
    \begin{defn}[Discrétisation de l'opérateur \(\LD\)]
      \begin{align*}
          \mLD = - \sum_{j=1}^N \int_{\Gamma_h} \vdivs \phij(\vx) \vdivs \phii(\vx) \dd{\Gamma}(\vx)
      \end{align*}
    \end{defn}
    Par définition des fonctions \(\phij\), on ne peut pas définir \( \int_{\Gamma_h} \LR(\phii)(\vx) \cdot \phij(\vx) \dd{\Gamma}(\vx)\) puisqu'elles ne sont pas dans \(\Hrot\).
...
\section{Forme matricielle de la discrétisation de la forme variationnelle des équations intégrales avec condition d'impédance}
  ...
  \begin{defn}
    ...
    \begin{align*}
      S_{pq}
        &= ik \int_{\Gamma_h} \left( \int_{\Gamma_h} g(\vx - \vy) \phip(\vy) \dd{\Gamma}(\vy) \right) \cdot \phiq (\vx)\dd{\Gamma}(\vx) \cdots\\
